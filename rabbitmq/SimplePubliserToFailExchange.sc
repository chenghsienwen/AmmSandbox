...
connection ! CreateChannel(ChannelActor.props(setupPublisher), Some("publishToFail"))
Thread.sleep(5000)
run()
Thread.sleep(5000)
def setupPublisher(channel: Channel, self: ActorRef) {
  val queue = channel.queueDeclare("segment_fail_queue_test", true, false, false, settings).getQueue
  channel.queueBind(queue, exchange, "segment_fail_queue_test")
}
...
def run (): Unit = {
  val publisher = system.actorSelection("/user/akka-rabbitmq/publishToFail")
  def publish(channel: Channel, index: Long) {
    channel.basicPublish(exchange, "segment_fail_queue_test", null, toBytes(index))
  }
  val msgs = 0L to 30L
  msgs.foreach { x =>
