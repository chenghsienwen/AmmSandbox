...
def setupPublisher(channel: Channel, self: ActorRef) {
  val queue = channel.queueDeclare("segment_work_queue", true, false, false, settings).getQueue
  channel.queueBind(queue, exchange, "segment_work_queue")
}
...
def run (): Unit = {
  ...
  def publish(channel: Channel, index: Long) {
    channel.basicPublish(exchange, "segment_work_queue", null, toBytes(index))
  }
  val msgs = 15L to 25L
  msgs.foreach { x =>
